...
export default function AdminReportsPage() {
    ...
    const filteredSales = useMemo(() => {
        ...
        return report.daily_sales.slice(-periodDays[period]);
    }, [report, period]);

    const categorySales = useMemo(() => {
        if (!report?.category_sales_total) return [];
        if (period === 'tahun_ini') return report.category_sales_total;
        const allowed = new Set(filteredSales.map(item => item.date));
        return (report.category_sales_daily || [])
            .filter(item => allowed.has(item.date))
            .reduce((result, item) => {
                const current = result.find(category => category.category === item.category);
                if (current) current.total_sales += Number(item.total_sales);
                else result.push({ category: item.category, total_sales: Number(item.total_sales) });
                return result;
            }, [])
            .sort((a, b) => b.total_sales - a.total_sales);
    }, [report, filteredSales, period]);
    ...
    const exportCsv = () => {
        const rows = [['Tanggal', 'Total Penjualan', 'Jumlah Transaksi'], ...filteredSales.map(item => [item.date, item.total_sales, item.total_orders])];
        const csv = rows.map(row => row.join(',')).join('\n');
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `laporan-penjualan-${period}.csv`;
        link.click();
    ...
    const lineData = {
        ...
        datasets: [{
            ...
            borderColor: '#dc2626', backgroundColor: 'rgba(220, 38, 38, 0.12)', fill: true,
            tension: 0.35, pointRadius: 3, pointHoverRadius: 6, pointBackgroundColor: '#dc2626', pointBorderColor: '#fff', pointBorderWidth: 2
        }]
    };
    ...
    const doughnutData = {
        ...
        datasets: [{ data: categorySales.map(item => item.total_sales), backgroundColor: ['#dc2626', '#2563eb', '#059669', '#d97706', '#7c3aed', '#0891b2'], borderWidth: 3, borderColor: '#fff' }]
    };

    if (loading) return <div className="flex items-center justify-center min-h-[60vh]"><RefreshCcw className="w-8 h-8 text-red-600 animate-spin" /></div>;
    if (error) return <div className="bg-red-50 border border-red-200 text-red-700 p-6 rounded-2xl flex flex-col items-center gap-4"><AlertTriangle className="w-10 h-10" /><p className="font-bold">{error}</p><button onClick={fetchReport} className="px-4 py-2 bg-red-600 text-white rounded-xl font-bold text-sm">Coba Lagi</button></div>;
        ...
        <div className="space-y-6">
            <header className="bg-slate-950 text-white rounded-3xl p-7 shadow-xl shadow-slate-900/10 relative overflow-hidden">
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-5">
                    <div><p className="text-red-300 text-xs font-black uppercase tracking-[0.2em]">Performance center</p><h2 className="text-3xl font-black tracking-tight mt-2">Laporan Penjualan</h2><p className="text-slate-300 text-sm mt-2">Baca ritme bisnis, temukan kategori kuat, dan pantau risiko stok.</p></div>
                    <div className="flex flex-wrap gap-3">
                        <label className="flex items-center gap-2 bg-white/10 border border-white/15 rounded-xl px-3 text-sm"><CalendarDays className="w-4 h-4 text-red-300" /><select value={period} onChange={event => setPeriod(event.target.value)} className="bg-transparent py-2.5 outline-none font-bold text-white"><option className="text-slate-900" value="hari_ini">Hari ini</option><option className="text-slate-900" value="minggu_ini">7 hari terakhir</option><option className="text-slate-900" value="bulan_ini">30 hari terakhir</option><option className="text-slate-900" value="tahun_ini">Tahun ini</option></select></label>
                        <button onClick={fetchReport} title="Refresh laporan" className="p-2.5 rounded-xl bg-white/10 hover:bg-white/20"><RefreshCcw className="w-4 h-4" /></button>
                        <button onClick={exportCsv} title="Export laporan" className="flex items-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-500 rounded-xl text-sm font-bold"><Download className="w-4 h-4" /> Export</button>
                    </div>
                </div><div className="absolute -right-12 -bottom-24 w-64 h-64 rounded-full border-[35px] border-red-600/20" />
            </header>
            ...
            <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div className="xl:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm"><div className="flex justify-between items-start mb-5"><div><h3 className="font-black text-slate-900">Ritme omzet</h3><p className="text-xs text-slate-500 mt-1">Pergerakan penjualan berdasarkan periode terpilih</p></div><BarChart3 className="w-5 h-5 text-red-600" /></div><div className="h-72">{filteredSales.length ? <Line data={lineData} options={chartOptions} /> : <div className="h-full flex items-center justify-center text-sm text-slate-400">Belum ada data penjualan.</div>}</div></div>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm"><div className="flex items-center gap-3 mb-5"><div className="p-2.5 rounded-xl bg-red-50 text-red-600"><Package className="w-5 h-5" /></div><div><h3 className="font-black text-slate-900">Kategori terkuat</h3><p className="text-xs text-slate-500">Prioritas perhatian pada periode ini</p></div></div><p className="text-3xl font-black text-slate-900">{topCategory?.category || '-'}</p><p className="text-sm text-slate-500 mt-1">{topCategory ? `${formatRupiah(topCategory.total_sales)} kontribusi omzet` : 'Belum ada penjualan'}</p></div>
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm"><div className="flex items-center gap-3 mb-5"><div className="p-2.5 rounded-xl bg-amber-50 text-amber-600"><AlertTriangle className="w-5 h-5" /></div><div><h3 className="font-black text-slate-900">Perlu ditindaklanjuti</h3><p className="text-xs text-slate-500">Stok paling kritis saat ini</p></div></div>{report?.low_stock_products?.slice(0, 3).map(product => <div key={product.id} className="flex justify-between items-center py-2 border-b border-slate-100 last:border-0"><span className="text-sm font-bold text-slate-700 truncate pr-3">{product.name}</span><span className="text-xs font-black text-red-600">{product.stock} tersisa</span></div>)}{!report?.low_stock_products?.length && <p className="text-sm text-slate-400">Semua stok dalam kondisi aman.</p>}</div>
            </section>
        </div>
    );
}
